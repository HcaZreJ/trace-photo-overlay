...
> 时长为**运动时长**（逐段累加、扣除停顿）；均速、配速随之计算。**单日轨迹通常较准；多日轨迹因导出 GPX 与 App 内部数据存在口径差异，时长/均速可能偏低，建议手改为 App 官方值。** 总爬升用移动平均平滑 + 迟滞阈值去噪（修正 GPS 海拔抖动导致的数倍虚高）。纯 GPS 计算与 App 的气压计 / 专有算法会有出入，数值均可手改。
